...
function PhaseBar({ phase, onJump }: { phase:ExamPhase; onJump?:(target:ExamPhase)=>void }) {
  ...
  return (
    <div style={{ display:'flex', alignItems:'center', gap:4 }}>
      {phases.map((ph,i) => { const pi=order.indexOf(ph); const done=cur>pi; const active=ph===phase||(phase==='parsing'&&ph==='upload'); const canJump = cur>=pi && phase!=='parsing'; return (
...
export default function ExamCreator({ activeClass, onBack, defaultSubject }: { activeClass:ClassInfo|null; onBack:()=>void; defaultSubject?:string }) {
  ...
  const runParse = async (file: File) => {
    setPhase('parsing'); setParseLog([]); setParseProgress(0); setParseError(null);
    ...
    try {
      ...
      parsed = data.questions || []; sub = data.subject||''; chp = data.chapter||'';
  ...
  const handleParseFromFile = async (file: File) => {
    try {
      ...
      setParseError(err?.message || 'Không parse được file theo mẫu K12');
      setPhase('upload');
      setParseProgress(0);
      setParseLog([]);
    }
  ...
  const resetAll = () => { setPhase('upload'); setBuildMode('choose'); setParseLog([]); setParseProgress(0); setQuestions([]); setParsedImages([]); setParseMatrix(null); setParseError(null); setBuilderQs([]); setManualAnswers({}); setCfg({ ...DEFAULT_CFG, subject: defaultSubject || DEFAULT_CFG.subject }); setSavedDraft(false); setExamId(null); setPublishSuccess(false); };
        ...
        <div style={{ animation:'examReveal .3s ease both' }}>
          ...
          {buildMode === 'file' && (
            <>
              ...
              {parseError && <div style={{ marginBottom:10, padding:'8px 10px', borderRadius:8, background:'rgba(248,113,113,.14)', border:'1px solid rgba(248,113,113,.35)', fontSize:12, color:'#fda4af' }}>⚠ {parseError}</div>}
        ...
        <div style={{ animation:'examReveal .3s ease both' }}>
          <div style={{ padding:20, borderRadius:12, background:'rgba(255,255,255,.04)', border:'1px solid rgba(255,255,255,.1)' }}>
            <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:14 }}>
              <div style={{ width:16, height:16, borderRadius:'50%', border:'2px solid #60C8FF', borderTopColor:'transparent', animation:'spin .8s linear infinite', flexShrink:0 }} />
              <span style={{ fontSize:13, fontWeight:600, color:'#E2EAF4' }}>Đang phân tích file...</span>
            </div>
            <div style={{ height:4, background:'rgba(255,255,255,.08)', borderRadius:2, marginBottom:14, overflow:'hidden' }}>
              <div style={{ height:'100%', width:`${parseProgress}%`, background:'#185FA5', borderRadius:2, transition:'width .4s ease' }} />
            </div>
            <div style={{ fontFamily:'monospace', fontSize:11, color:'rgba(255,255,255,.5)', lineHeight:1.8 }}>
